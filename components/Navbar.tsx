...
export default function Navbar() {
  ...
  const navLinks = [
    { 
      name: "Industries", 
      href: "/industries",
      dropdown: ["Critical Infrastructure", "Grid Edge", "Commercial & Industrial", "Remote Power"]
    },
    { 
      name: "Products", 
      href: "/products",
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="w-[30%] flex items-center justify-start">
          <Link href="/" className="relative h-12 w-48 md:h-16 md:w-64 flex-shrink-0">
             <Image
        ...
        <div className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <div key={link.name} className="relative group">
              <button 
                className={`flex items-center gap-3 text-lg font-medium transition-colors text-gray-800 hover:text-primary`}
              >
                {link.name}
          ...
          <div className="flex flex-col p-4 space-y-4">
            {navLinks.map((link) => (
              <div key={link.name} className="border-b border-gray-100 pb-2 last:border-0">
                <div className="font-semibold text-gray-900 mb-2">{link.name}</div>
